...
<div>
<button type="button" class="btn btn-sm" data-toggle="modal" data-target="#employeeModal">Create New Employee</button>
</div>
...
<div class="col-xs-6">
  <form method="post" enctype="multipart/form-data" action="{{ url('/employee/import') }}">
     @csrf
     <input type="file" id="uploadName" name="employee_import" required>
 </div>
 
   @error('employee_import')
     <small>{{ $message }}</small>
   @enderror
...
<div class="modal " id="employeeModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
   ...
   <span aria-hidden="true">&times;</span>
        ...
        <form  method="POST" action="{{ route('user.esignup') }}" enctype="multipart/form-data">
        {{csrf_field()}}
